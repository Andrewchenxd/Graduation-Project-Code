...
class ICAMCNET(nn.Module):
    def __init__(self, num_classes,adsb_is=False):
        ...
        if adsb_is == False:
            ...
            self.fc1 = nn.Sequential(nn.Linear(in_features=32000, out_features=256),
                                     nn.ReLU(),
                                     )
...
if __name__ == '__main__':
    import time
    net1= ICAMCNET(10,True).cuda()
    total = 32
    a = torch.randn((total, 1, 128, 128)).cuda()
    b = torch.randn((total,1, 2, 128)).cuda()
