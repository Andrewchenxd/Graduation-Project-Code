...
# 获取当前文件的父目录的父目录（即 SignalGPT 目录）
project_root = Path(__file__).parent.parent
sys.path.append(str(project_root))  # 添加 SignalGPT 到 Python 路径
...
from tool.decoder import Decoder, DecoderLayer
from tool.embed import DataEmbedding
from tool.attn import FullAttention, ProbAttention, AttentionLayer,FreqAttention
from tool.resnet_fc import ResNet1d_enc
from tool.embed import PatchEmbedding,PatchEmbedding_hf
import transformers
from tool.StandardNorm import Normalize
import einops
import math
from tool.utils import replace_lora_layer_with_custom
...
def soft_hard_prompt(prompt_embeddings,prompt_word_embeddings,K):
    ...
    topk_indices = topk_indices.unsqueeze(-1).expand(-1, -1, D)
    # 从 word_emd 中收集 top-k 词嵌入向量
    prompt_embeddings = torch.gather(prompt_word_embeddings.expand(B, -1, -1), 1, topk_indices)
...
class NeuralWeightedFusion(nn.Module):
    def __init__(self, d_model: int = None, hidden: int = None, dropout: float = 0.1, use_norm: bool = True):
        super().__init__()
        self._lazy = d_model is None
        self.gate = None
        self.norm = None
        self.hidden = hidden
        self.dropout = dropout
        self.use_norm = use_norm
        self._d_model = d_model  # 允许预先指定

    def _build(self, d_model: int, device=None, dtype=None):
        hidden = self.hidden or max(64, d_model // 2)
        ...
        self.norm = nn.LayerNorm(d_model) if self.use_norm else nn.Identity()
        # 关键：将新创建的子模块移动到与输入一致的 device/dtype
    ...
    def forward(self, A: torch.Tensor, B: torch.Tensor) -> torch.Tensor:
        assert A.shape == B.shape, f"fusion shape mismatch {A.shape} vs {B.shape}"
        permuted = False

        Bsz, L, D = A.shape
        if self._lazy:
            self._build(D, device=A.device, dtype=A.dtype)
        Z = torch.cat([A, B, A - B, A * B], dim=-1)  # (B,L,4D)
        g = self.gate(Z)                              # (B,L,1)
        Y = g * A + (1.0 - g) * B                     # (B,L,D)
        Y = self.norm(Y)
        return Y.permute(0, 2, 1) if permuted else Y


class RadioLLM_RAG_HNSW(nn.Module):

    def __init__(self, configs, yaml_config):
        super(RadioLLM_RAG_HNSW, self).__init__()
        ...
        self.dataset_name=['RML2016a+b_total_snr','RML2018_high_snr','ADSB','WIFI_2ft']

        with open(r'D:\planedemo\SignalGPT\result\HNSW_save\all_datasets_hnsw_with_features.pkl', 'rb') as f:
            _loaded = pickle.load(f)

        # 标准结构：{'indices': {ds: bytes}, 'features': {ds: np.ndarray}}
        if isinstance(_loaded, dict) and ('indices' in _loaded or 'features' in _loaded):
            self.all_serialized = _loaded.get('indices', {})
            self.all_features = _loaded.get('features', None)  # 可为 None
        else:
            # 兼容旧结构：可能直接就是 {ds: bytes}
            self.all_serialized = _loaded
            self.all_features = None
        del _loaded
        # HNSW 索引缓存与默认搜索参数
        self._faiss_cache = {}
        self.hnsw_ef_search = 64  # 可在外部覆盖
        self.hnsw_fusion = NeuralWeightedFusion(d_model=None, dropout=0.1, use_norm=True)
    ...
    def _get_faiss_index(self, name: str):
        if name in self._faiss_cache:
            return self._faiss_cache[name]
        if hasattr(self, "all_serialized") and (name in self.all_serialized):
            idx = faiss.deserialize_index(self.all_serialized[name])
        elif hasattr(self, "hnsw_indices") and (name in self.hnsw_indices):
            obj = self.hnsw_indices[name]
            idx = faiss.deserialize_index(obj) if isinstance(obj, (bytes, bytearray)) else obj
        else:
            raise RuntimeError(f"HNSW index for dataset '{name}' not found.")
        try:
            idx.hnsw.efSearch = self.hnsw_ef_search
        except Exception:
            pass
        self._faiss_cache[name] = idx
        return idx

    @torch.no_grad()
    def _faiss_search_topk(self, q_flat_np: np.ndarray, dataset_name: str, k: int):
    ...
    def HNSW_RAG(self, enc_out_sgn_high: torch.Tensor, dataset_name: str, k: int = 5) -> torch.Tensor:
        """
        对 (B,D,L) 特征进行 HNSW 检索与加权聚合，返回 (B,D,L)；对查询保持可导，邻居选择不可导。
        低内存：逐样本重构 k 个邻居。
        """
        x = enc_out_sgn_high
        ...
        q_flat = x.view(B, -1)  # (B, D*L), 保留计算图
        # 检索用 CPU float32 NumPy
        q_np = q_flat.detach().to("cpu", dtype=torch.float32).contiguous().numpy()

        index, dist, nn_indices = self._faiss_search_topk(q_np, dataset_name, k)

        out = q_flat.new_zeros((B, dim))  # (B, D*L)
        # 逐样本聚合，常数级内存
        for i in range(B):
            ids = nn_indices[i].tolist()
            # 重构邻居 -> (k, dim) torch
            neigh = np.stack([index.reconstruct(int(j)) for j in ids], axis=0)  # float32
            V = torch.from_numpy(neigh).to(device=device, dtype=dtype)          # (k, dim)

            qi = q_flat[i]                              # (dim,)
            di = (V - qi.unsqueeze(0)).pow(2).sum(-1)   # (k,)
            w = torch.softmax(-di, dim=-1)              # (k,)
            out[i] = (w.unsqueeze(1) * V).sum(0)        # (dim,)

        return out.view(B, D, L)
    
    def fuse_with_hnsw(self, enc_out_sgn_high: torch.Tensor, dataset_name: str, k: int = 5):
        """
        先用 HNSW 强化 enc_out_sgn_high，然后与 enc_out 做可学习融合。
        enc_out, enc_out_sgn_high 可为 (B,L,D) 或 (B,D,L)，自动适配。
        返回与 enc_out 同布局的张量。
        """
        # 统一得到 (B,D,L) 的 HNSW 聚合结果
        hnsw_enh = self.HNSW_RAG(enc_out_sgn_high, dataset_name, k=k)  # (B,D,L)

        # 将 enc_out / hnsw_enh 统一为 (B,L,D) 做融合
        restore_layout = None
        if enc_out_sgn_high.dim() == 3 and enc_out_sgn_high.shape[1] == hnsw_enh.shape[2]:  # enc_out:(B,L,D?) hnsw:(B,D,L)
            # enc_out 是 (B,L,D)，hnsw_enh 转到 (B,L,D)
            A = enc_out_sgn_high
            B = hnsw_enh.permute(0, 2, 1)
            restore_layout = "BLD"
        elif enc_out_sgn_high.dim() == 3 and enc_out_sgn_high.shape[1] == hnsw_enh.shape[1]:  # enc_out:(B,D,L)
            A = enc_out_sgn_high.permute(0, 2, 1)
            B = hnsw_enh.permute(0, 2, 1)
            restore_layout = "BDL"
        else:
            raise RuntimeError(f"Unrecognized shapes for fusion: enc_out={enc_out_sgn_high.shape}, hnsw={hnsw_enh.shape}")

        fused = self.hnsw_fusion(A, B)  # (B,L,D)
        if restore_layout == "BLD":
            return fused
        else:
    ...
    def forecast(self, x_enc, enable_mask=False, dataset_name=None, encoder_k_is=False):
        ...
        enc_out_sgn_high = self.high_freq_extract(x_enc)
        enc_out_sgn_high = self.fuse_with_hnsw(enc_out_sgn_high, dataset_name)
        
        enc_out_sgn_high = einops.rearrange(enc_out_sgn_high, 'b (n d) l -> (b n) l d',n=n_vars)

        enc_out = self.reprogramming_layer(enc_out_sgn, source_embeddings, source_embeddings)
        enc_out = self.attn_fusion(enc_out, enc_out_sgn_high)
    ...
    def classific(self, x_enc, enable_mask=False, dataset_name=None, encoder_k_is=False):
        ...
        enc_out_sgn, n_vars = self.patch_embedding(x_enc)
        enc_out_sgn_high = self.high_freq_extract(x_enc_ori)
        enc_out_sgn_high = self.fuse_with_hnsw(enc_out_sgn_high, dataset_name)
        enc_out_sgn_high = einops.rearrange(enc_out_sgn_high, 'b (n d) l -> (b n) l d',n=n_vars)
...
class CNN_RAG(nn.Module):
    def __init__(self, configs):
        super(CNN_RAG, self).__init__()
        self.d_llm=configs.llm_dim
        self.patch_len = configs.patch_len
        self.stride = configs.stride
        self.normalize_layers = Normalize(configs.enc_in, affine=False)
        ...
        self.hnsw_fusion = NeuralWeightedFusion(d_model=None, dropout=0.1, use_norm=True)
        self.pool = nn.AdaptiveAvgPool1d(1)
        self.mlp=nn.Sequential(nn.Linear(self.d_llm*2, 128),
                               nn.ReLU(),
                               nn.Dropout(0.1),
                               nn.Linear(128, configs.numclass))
    ...
    def _build_axis_indices(self, dataset_name: str, D: int, L: int):
        """
        构建并缓存展平索引 (N, D*L)，仅一次。用于后续检索 M 个邻居。
        需要 self.all_features[dataset_name] 形状 (N,D,L) 或 (N,D*L)。
        """
        if not hasattr(self, "_flat_index_cache"):
            self._flat_index_cache = {}
            self._flat_feats = {}
        if dataset_name in self._flat_index_cache:
            return self._flat_index_cache[dataset_name], self._flat_feats[dataset_name]

        if self.all_features is None or dataset_name not in self.all_features:
            raise RuntimeError(f"缺失特征: {dataset_name}")

        feats = self.all_features[dataset_name]  # (N,D,L) 或 (N,D*L)
        N = feats.shape[0]
        if feats.ndim == 2 and feats.shape[1] == D * L:
            flat_feats = feats.astype('float32')
        elif feats.ndim == 3 and feats.shape[1] == D and feats.shape[2] == L:
            flat_feats = feats.reshape(N, D * L).astype('float32')
        else:
            raise RuntimeError("特征需为 (N,D,L) 或展平 (N,D*L)")

        index = faiss.IndexFlatL2(D * L)
        index.add(flat_feats)

        self._flat_index_cache[dataset_name] = index
        self._flat_feats[dataset_name] = flat_feats  # 保留原始展平矩阵，避免 reconstruct
        return index, flat_feats

    def multi_axis_hnsw_fusion(self, x: torch.Tensor, dataset_name: str, M_keys: int = 32) -> torch.Tensor:
        assert x.dim() == 3
        B, D, L = x.shape
        index, flat_feats = self._build_axis_indices(dataset_name, D, L)

        # 防止请求超过库大小
        M_keys = min(M_keys, index.ntotal)
        if M_keys < 4:
            raise ValueError("可用邻居数不足 4")

        x_flat = x.view(B, D * L)
        q_np = x_flat.detach().to("cpu", dtype=torch.float32).contiguous().numpy()

        # 正确解包
        dist, nn_ids = index.search(q_np, M_keys)  # dist:(B,M_keys), nn_ids:(B,M_keys)

        mid_pos = M_keys // 2
        fused_list = []
        for b in range(B):
            neigh_flat = flat_feats[nn_ids[b]]  # (M_keys, D*L)
            neigh = torch.from_numpy(neigh_flat.reshape(M_keys, D, L)).to(x.device, dtype=x.dtype)
            xb = x[b]

            dist_D = (neigh - xb.unsqueeze(0)).pow(2).sum(-1)          # (M_keys,D)
            sim_idx_D = dist_D.argmin(0)
            sorted_idx_D = dist_D.argsort(0)
            med_idx_D = sorted_idx_D[mid_pos, torch.arange(D)]
            sim_D = neigh[sim_idx_D, torch.arange(D)]
            med_D = neigh[med_idx_D, torch.arange(D)]

            dist_L = (neigh.transpose(1, 2) - xb.transpose(0, 1).unsqueeze(0)).pow(2).sum(-1)  # (M_keys,L)
            sim_idx_L = dist_L.argmin(0)
            sorted_idx_L = dist_L.argsort(0)
            med_idx_L = sorted_idx_L[mid_pos, torch.arange(L)]
            sim_L = neigh[sim_idx_L, :, torch.arange(L)].transpose(0, 1)
            med_L = neigh[med_idx_L, :, torch.arange(L)].transpose(0, 1)

            def to_BLD(t): return t.unsqueeze(0).permute(0, 2, 1)
            orig = to_BLD(xb)
            f = self.hnsw_fusion(orig, to_BLD(sim_D))
            f = self.hnsw_fusion(f, to_BLD(med_D))
            f = self.hnsw_fusion(f, to_BLD(sim_L))
            f = self.hnsw_fusion(f, to_BLD(med_L))
            fused_list.append(f.squeeze(0).permute(1, 0))
    ...
    def forward(self, x_enc, enable_mask=False, dataset_name=None):
        ...
        x_enc = x_enc.permute(0, 2, 1)
        enc_out_sgn_high = self.high_freq_extract(x_enc)
        enc_out_sgn_high = self.multi_axis_hnsw_fusion(enc_out_sgn_high, dataset_name,10)
        out=self.pool(enc_out_sgn_high).view(enc_out_sgn_high.size(0), -1)
        out=self.mlp(out)
...
if __name__ == '__main__':
    ...
    file_path = r"D:\planedemo\SignalGPT\data\communication_data.jsonl"
    ...
    # 读取YAML文件
    with open(r'D:\planedemo\SignalGPT\scripts\multi_task_pretrain.yaml', 'r') as file:
        yaml_config = yaml.safe_load(file)
    # recall_path= r'D:\planedemo\SignalGPT\pretrain_model\Qwen3-Embedding-0.6B'
    # recall_model=SentenceTransformer(recall_path)
    
    # embeddings_prompt = recall_model.encode(Prompt_list, normalize_embeddings=True)
    
    model = RadioLLM_
    RAG_HNSW(args, yaml_config=yaml_config)
    # model =CNN_RAG(args)
    ...
    #         del module.total_ops  # 删除冲突的属性
    # flops, params = profile(model, inputs=(x_enc,), verbose=False, ret_layer_info=True)
