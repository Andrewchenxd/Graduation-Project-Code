...
class SKUnit(nn.Module):
    def __init__(self, in_channels, mid_channels, out_channels, branches=2, group=32, reduce=16, stride=1, len=32):
        ...
        if in_channels == out_channels and stride==1:  # when dim not change, input_features could be added directly to out
...
class Multi_spectral_attention_mechanism(nn.Module):
    ...
    def __init__(self, input_channel=32):
        ...
        self.cnn = nn.Conv1d(input_channel, input_channel, kernel_size=1, stride=1, padding=0, bias=False)
        self.fc=nn.Linear(input_channel,1)
    def forward(self, x):
        DCT =  torch.abs(torch.fft.fft2(x))
        DCT_res=self.cnn(DCT)
        score=torch.nn.functional.gumbel_softmax(DCT_res,tau=0.01)
        DCT=DCT*score
        DCT=torch.transpose(DCT,1,2)
...
class FLAN(nn.Module):
    def __init__(self, num_classes, num_block_lists=[3, 4, 6, 3]):
        ...
        self.stage_1 = self._make_layer(2, 32, 32, nums_block=num_block_lists[0], stride=1)
        self.stage_2 = self._make_layer(32, 32, 32, nums_block=num_block_lists[1], stride=2)
        self.stage_3 = self._make_layer(32, 32, 32, nums_block=num_block_lists[2], stride=2)
        self.stage_4 = self._make_layer(32, 32, 32, nums_block=num_block_lists[3], stride=2)
        self.fre_attn=Multi_spectral_attention_mechanism(input_channel=32)
        self.gap = nn.AdaptiveAvgPool1d(1)
...
if __name__ == '__main__':
    net1= FLAN(10,64).cuda()
    a=torch.randn((2,1,128,128)).cuda()
    b=torch.randn((2,2,128)).cuda()
    c=torch.randn((2,1,17)).cuda()
