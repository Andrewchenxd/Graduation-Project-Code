"""
数据模块 - RML2016a数据集加载与预处理

提供数据集类，支持RML2016.10a/10b/10c和ADSB数据集的加载与预处理。
数据格式为npy文件，包含特征X和标签Y。
...
数据目录结构:
    ...
    │   ├── test_X.npy
    │   └── test_Y.npy
    ├── {dataset}_train_data.npy
    ├── {dataset}_train_label.npy
    ├── {dataset}_train_snr.npy
    ├── {dataset}_val_data.npy
    ├── {dataset}_val_label.npy
    ├── {dataset}_val_snr.npy
    ├── {dataset}_test_data.npy
    ├── {dataset}_test_label.npy
    └── {dataset}_test_snr.npy
"""
...
class SigDataSet_stft(Dataset):
    """
    基于短时傅里叶变换(STFT)的信号数据集

    支持RML2016系列数据集和ADSB数据集
    """

    def __init__(self, data_path, adsbis=False, newdata=False, snr_range=[1, 7],
                 resample_is=False, samplenum=15, norm='maxmin',
                 chazhi=False, chazhinum=2, is_DAE=False):
        super().__init__()
...
class SigDataSet_pwvd(Dataset):
    """
    基于伪Wigner-Ville分布(PWVD)的信号数据集

    支持RML2016系列数据集和ADSB数据集
    """

    def __init__(self, data_path, adsbis=False, snr_range=[1, 7],
                 resample_is=False, samplenum=15, norm='no',
                 chazhi=False, chazhinum=2, is_DAE=False,
                 resize_is=False, aug_is=False):
        super().__init__()
        # data_path 为 .npy 文件前缀（不含 _data.npy 后缀）
        self.data = np.load(data_path + '_data.npy')
        self.labels = np.load(data_path + '_label.npy')
